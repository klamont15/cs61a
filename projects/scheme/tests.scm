...
(quote (car (cdr (cons 1 2))))
; expect (car (cdr (cons 1 2)))
...
(begin (display 6) (newline) (+ 1 2) (* 5 4))
; expect 6 ; 20
...
(define (replace lst old new)
  (cond
    ((null? lst) nil)
    ((= (car lst) old) (cons new (replace (cdr lst) old new)))
    (else (cons (car lst) (replace (cdr lst) old new)))
  )
)

(define a (cons 1 (cons 2 (cons 2 (cons 3 nil)))))
...
(define (has-element lst elem)
  (cond
    ((null? lst) False)
    ((= (car lst) elem) True)
    (else (has-element (cdr lst) elem))
  )
)
...
(define (hailstone x)
       (cond
         ((= x 1) 1)
         ((= (remainder x 2) 0) (display x) (newline) (hailstone (/ x 2)))
         (else (display x) (newline) (hailstone (+ (* 3 x) 1)))
       )
     )
(hailstone 5)
; expect 5 ;16 ;8 ;4 ;2 ;1

(define (eight-bit num pow)
  (cond
    ((= pow 0) (remainder num 2))
    ((or (< (expt 2 pow) num) (= (expt 2 pow) num)) (display 1) (eight-bit (- num (expt 2 pow)) (- pow 1)))
    (else (display 0) (eight-bit num (- pow 1)))
  )
)
...
(define (multiply-terms function a b)
  (if (= b a)
    (function a)
    (* (function b) (multiply-terms function a (- b 1)))
  )
)
...
(multiply-terms identity 4 10)
;expect 604800

(multiply-terms square 2 5)
;expect 14400

(identity (multiply-terms square 2 5))
;expect 14400

(if (identity 0) 2 4)
;expect 2

(if (identity False) 2 4)
;expect 4

(and 3 4 5 0 1 2 False 4 5 (/ 1 0) 5)
;expect False
...
(define (roots a b c)
  (begin
    (define discriminant (expt (- (expt b 2) (* 4 a c)) .5))
    (cons (/ (+ (- b) discriminant) (* 2 a)) (cons (/ (- (- b) discriminant) (* 2 a)) nil))
  )
)
