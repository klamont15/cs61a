...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        mapped_rest = rest.map(lambda value: scheme_eval(value, env))
        return scheme_apply(proc, mapped_rest, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        def frame_binder(frame, formals, vals):
            if formals is nil and vals is nil:
                return
            try:
                frame.define(formals.first, vals.first)
                frame_binder(frame, formals.second, vals.second)
            except AttributeError as e:
                raise SchemeError
        frame_binder(child, formals, vals)
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of parameters')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        name = target.first
        parameters = target.second
        lambda_expression = do_lambda_form(Pair(parameters, expressions.second), env)
        env.define(name, lambda_expression)
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    first, pair = expressions.first, expressions.second
    if pair is nil:
        return scheme_eval(first, env, True)
    evaluation = scheme_eval(first, env)
    if scheme_falsep(evaluation):
        return False
    return do_and_form(pair, env)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    if pair is nil:
        return scheme_eval(first, env, True)
    evaluation = scheme_eval(first, env)
    if scheme_truep(evaluation):
        return evaluation
    return do_or_form(pair, env)
    # END PROBLEM 13
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                try:
                    ...
                    raise SchemeError
            elif isinstance(clause.second.second, Pair):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError ('cannot bind value to this type')
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            proc = scheme_eval(first, env)
            mapped_rest = rest.map(lambda value: scheme_eval(value, env))
            result = scheme_apply(proc, mapped_rest, env)
            #END
    return result
