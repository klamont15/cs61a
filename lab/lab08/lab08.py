...
def make_to_string(front, mid, back, empty_repr):
    ...
    '()'
    """
    def string_function(lnk):
        if lnk is ():
            return empty_repr
        return front + str(lnk.first) + mid + make_to_string(front, mid, back, empty_repr)(lnk.rest) + back
...
def add_trees(t1, t2):
    ...
    4
      ...
      5
    """
    a = len(t1.branches) - len(t2.branches)
    list1, list2 = t1.branches[:], t2.branches[:]
    if a > 0:
      for i in range(a):
        list2.append(Tree(0))
    if a < 0:
      for i in range(-a):
        list1.append(Tree(0))
    return Tree(t1.root + t2.root,[add_trees(subtree1, subtree2) for subtree1, subtree2 in zip(list1, list2)])
